...
public class MainActivity extends Activity {
    ...
    FrameLayout root; LinearLayout content; String exam="NEET"; long userId=-1; String authToken=""; int qIndex=0,score=0,colorfulIndex=0; long duration=30; long remainingSeconds=0; CountDownTimer timer;
    ...
    String pendingProfileName="",pendingProfileEmail="",pendingProfileCreatedAt="";
    ...
    void saveSession(long id,String token){ userId=id; authToken=token; getSharedPreferences("crackneet_session",MODE_PRIVATE).edit().putLong("userId",id).putString("token",token).putString("name",profileNameSafe(id)).putString("email",profileEmailSafe(id)).putString("createdAt",pendingProfileCreatedAt).apply(); }
    ...
    void login(final String email,final String password){
        ...
        new Thread(() -> {
            try{
                ...
                JSONObject u=out.getJSONObject("user"); pendingProfileName=u.optString("name","Student"); pendingProfileEmail=u.optString("email",""); pendingProfileCreatedAt=u.optString("created_at",""); saveSession(u.getLong("id"),out.getString("token"));
    ...
    void register(final String name,final String email,final String password){
        ...
        new Thread(() -> {
            try{
                ...
                JSONObject u=out.getJSONObject("user");pendingProfileName=u.optString("name","Student"); pendingProfileEmail=u.optString("email",""); pendingProfileCreatedAt=u.optString("created_at","");saveSession(u.getLong("id"),out.getString("token"));runOnUiThread(()->showDashboard());
...
void statBox(LinearLayout row,String icon,String label,String value,int color){
    ...
    void startNcertSampleTest(){
        ...
        n.add(new Question("NCERT-002","NEET","Biology","Biomolecules","NCERT Based","Moderate","Which class of biomolecules includes enzymes in most biological systems?",new String[]{"Proteins","Lipids","Minerals","Nucleic acids only"},0,"Most enzymes are proteins; they act as biological catalysts."));
        n.add(new Question("NCERT-003","NEET","Biology","Cell Cycle and Cell Division","NCERT Based","Moderate","If a diploid cell has 20 chromosomes, the number of chromosomes in each daughter cell after mitosis is:",new String[]{"20","10","40","5"},0,"Mitosis maintains chromosome number, so each daughter cell retains 20 chromosomes."));
        n.add(new Question("NCERT-004","NEET","Biology","Plant Kingdom","NCERT Based","Easy","Which group is characterised by embryos that develop within the female sex organ and by a dominant gametophyte in the life cycle?",new String[]{"Bryophytes","Gymnosperms","Angiosperms","Pteridophytes only"},0,"Bryophytes have a dominant gametophyte and the embryo remains attached to and nourished by the parent gametophyte."));
        n.add(new Question("NCERT-005","NEET","Biology","Human Reproduction","NCERT Based","Moderate","In humans, fertilisation normally occurs in the:",new String[]{"Ampullary-isthmic junction of the oviduct","Uterus","Cervix","Vagina"},0,"NCERT describes the ampullary-isthmic junction of the oviduct as the usual site of fertilisation."));
