...
async function auth(req,res,next){
 ...
 const {rows}=await pool.query("SELECT u.id,u.name,u.email,u.created_at,s.token_hash FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.token_hash=$1 AND s.expires_at>NOW()",[hash]);
...
app.post("/api/auth/register",async(req,res)=>{
 try{
  ...
  try{await pool.query("SELECT 1 FROM users LIMIT 1");}catch(dbErr){console.error("Registration DB readiness check failed",dbErr);await ensureDatabase();}
  ...
  const u=await pool.query("INSERT INTO users(name,email,password_hash) VALUES($1,$2,$3) RETURNING id,name,email,created_at",[cleanName,normalized,hash]);
...
app.post("/api/auth/login",async(req,res)=>{
 try{
  ...
  const {rows}=await pool.query("SELECT id,name,email,password_hash,created_at FROM users WHERE email=$1",[String(email).trim().toLowerCase()]);
  ...
  res.json({user:{id:rows[0].id,name:rows[0].name,email:rows[0].email,created_at:rows[0].created_at},token});
...
app.get("/api/users/me",auth,(req,res)=>res.json({user:{id:req.user.id,name:req.user.name,email:req.user.email,created_at:req.user.created_at}}));
const razorpay = process.env.RAZORPAY_KEY_ID && process.env.RAZORPAY_KEY_SECRET ? new (require("razorpay"))({key_id:process.env.RAZORPAY_KEY_ID,key_secret:process.env.RAZORPAY_KEY_SECRET}) : null;
const PLANS={basic:{amount:1900,questions:10000},moderate:{amount:4900,questions:20000},advance:{amount:9900,questions:null}};
app.post("/api/payments/create-order",auth,async(req,res)=>{try{if(!razorpay)return res.status(503).json({error:"razorpay_not_configured"});const plan=String(req.body?.plan||"").toLowerCase();if(!PLANS[plan])return res.status(400).json({error:"invalid_plan"});const p=PLANS[plan];const order=await razorpay.orders.create({amount:p.amount,currency:"INR",receipt:"cn_"+req.user.id+"_"+Date.now(),notes:{user_id:String(req.user.id),plan}});await pool.query("INSERT INTO subscriptions(user_id,plan,order_id,amount_paise,status) VALUES($1,$2,$3,$4,'created') ON CONFLICT(order_id) DO NOTHING",[req.user.id,plan,order.id,p.amount]);res.json({orderId:order.id,amount:p.amount,currency:"INR",plan,questions:p.questions,keyId:process.env.RAZORPAY_KEY_ID});}catch(e){console.error("Create order error",e);res.status(500).json({error:"order_creation_failed"});}});
app.post("/api/payments/verify",auth,async(req,res)=>{try{const {razorpay_order_id,razorpay_payment_id,razorpay_signature}=req.body||{};if(!razorpay_order_id||!razorpay_payment_id||!razorpay_signature||!process.env.RAZORPAY_KEY_SECRET)return res.status(400).json({error:"invalid_payment_verification"});const expected=crypto.createHmac("sha256",process.env.RAZORPAY_KEY_SECRET).update(razorpay_order_id+"|"+razorpay_payment_id).digest("hex");if(expected!==razorpay_signature)return res.status(400).json({error:"invalid_payment_signature"});const r=await pool.query("UPDATE subscriptions SET status='paid',payment_id=$1,updated_at=NOW() WHERE order_id=$2 AND user_id=$3 RETURNING id,plan,amount_paise,status,created_at",[razorpay_payment_id,razorpay_order_id,req.user.id]);if(!r.rows.length)return res.status(404).json({error:"order_not_found"});res.json({ok:true,subscription:r.rows[0]});}catch(e){res.status(500).json({error:"payment_verification_failed"});}});
app.get("/api/users/me/subscription",auth,async(req,res)=>{try{const {rows}=await pool.query("SELECT id,plan,amount_paise,status,payment_id,created_at FROM subscriptions WHERE user_id=$1 ORDER BY created_at DESC LIMIT 1",[req.user.id]);res.json({subscription:rows[0]||null});}catch(e){res.status(500).json({error:"subscription_fetch_failed"});}});
app.delete("/api/auth/account",auth,async(req,res)=>{const client=await pool.connect();try{await client.query("BEGIN");await client.query("DELETE FROM test_attempts WHERE user_id=$1",[req.user.id]);await client.query("DELETE FROM sessions WHERE user_id=$1",[req.user.id]);await client.query("DELETE FROM users WHERE id=$1",[req.user.id]);await client.query("COMMIT");res.json({ok:true});}catch(e){await client.query("ROLLBACK").catch(()=>{});res.status(500).json({error:"account_deletion_failed"});}finally{client.release();}});
...
ensureDatabase().then(()=>app.listen(PORT,()=>console.log("CrackNEET API listening on "+PORT))).catch(e=>{console.error("Startup database error",e);app.listen(PORT,()=>console.log("CrackNEET API listening on "+PORT));});
